<article id="post-<?php the_ID(); ?>" <?php post_class('vu_blog-post vu_bp-type-video'); ?> itemscope="itemscope" itemtype="https://schema.org/BlogPosting">
	<?php $housico_post_format_settings = housico_get_post_meta( $post->ID, 'housico_post_format_settings' ); ?>

	<?php if( !empty($housico_post_format_settings['video']['embed-code']) ) : ?>
		<div class="vu_bp-image">
			<?php 
				$shortcode_atts = array(
					"src" => esc_attr($housico_post_format_settings['video']['embed-code'])
				);

				echo do_shortcode( housico_generate_shortcode('vu_embed', $shortcode_atts) );
			?>
		</div>
	<?php elseif( !empty($housico_post_format_settings['video']['m4v']) or !empty($housico_post_format_settings['video']['ogg']) ) : ?>
		<div class="vu_bp-image">
			<div class="embed-responsive embed-responsive-2by1">
				<video class="embed-responsive-item" controls poster="<?php echo esc_url($housico_post_format_settings['video']['poster']); ?>">
					<?php if( !empty($housico_post_format_settings['video']['m4v']) ) : ?>
						<source src="<?php echo esc_url($housico_post_format_settings['video']['m4v']); ?>" type="video/mp4">
					<?php endif; ?>
					<?php if( !empty($housico_post_format_settings['video']['ogg']) ) : ?>
						<source src="<?php echo esc_url($housico_post_format_settings['video']['ogg']); ?>" type="video/ogg">
					<?php endif; ?>
					<?php esc_html_e('Your browser does not support the video tag.', 'housico'); ?>
				</video>
			</div>
		</div>
