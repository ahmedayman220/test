<article id="post-<?php the_ID(); ?>" <?php post_class('vu_blog-post vu_bp-type-audio'); ?> itemscope="itemscope" itemtype="https://schema.org/BlogPosting">
	...
	<?php if( !empty($housico_post_format_settings['audio']['mp3-file-url']) or !empty($housico_post_format_settings['audio']['oga-file-url']) ) : ?>
		<div class="vu_bp-image">
			<audio class="wp-audio-shortcode" style="width: 100%; visibility: hidden;" preload="none" controls="controls">
				<?php if( !empty($housico_post_format_settings['audio']['oga-file-url']) ) : ?>
					<source src="<?php echo esc_url($housico_post_format_settings['audio']['oga-file-url']); ?>" type="audio/ogg">
				<?php endif; ?>
				<?php if( !empty($housico_post_format_settings['audio']['mp3-file-url']) ) : ?>
					<source src="<?php echo esc_url($housico_post_format_settings['audio']['mp3-file-url']); ?>" type="audio/mpeg">
				<?php endif; ?>	
				<?php echo esc_html__('Your browser does not support the audio element.', 'housico'); ?>
			</audio>
		</div>
