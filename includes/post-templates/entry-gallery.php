<article id="post-<?php the_ID(); ?>" <?php post_class('vu_blog-post vu_bp-type-gallery'); ?> itemscope="itemscope" itemtype="https://schema.org/BlogPosting">
	<?php $housico_post_format_settings = housico_get_post_meta( $post->ID, 'housico_post_format_settings' ); ?>

	<?php if( !empty($housico_post_format_settings['gallery']['images']) ) : ?>
		<div class="vu_bp-image">
			<?php 
				$shortcode_atts = array(
					"_images" => $housico_post_format_settings['gallery']['images'],
					"ratio" => "2:1",
					"lightbox" => "1",
					"navigation" => "hover",
					"pagination" => "always",
				...
				echo do_shortcode( housico_generate_shortcode('vu_image_slider', $shortcode_atts) );
			?>
		</div>
