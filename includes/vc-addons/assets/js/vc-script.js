(function($) {
	...
	$(document.body).on('click.vu_vcOpenMediaManager', '.vu_param_m-btn[data-control="upload"]', function(e){
		e.preventDefault();
		...
		vu_vc_media_frame.on('select', function(){
			var $container = $this.parents('.vu_param_media'),
				media_attachment = vu_vc_media_frame.state().get('selection').first().toJSON();

			$container.find('.vu_param_m-img-id').val(media_attachment.id).trigger('change');

			$container.addClass('vu_has-img').find('.vu_param_m-img-url').val(media_attachment.url).trigger('change');
			$container.find('.vu_param_m-img').css({'background-image': 'url('+ media_attachment.url +')'});
			...
			if( $this.hasClass('vu_as-param') ) {
				...
				$container.find('.vu_param_m-value').val(window.btoa(unescape(encodeURIComponent(JSON.stringify(value)))));
			}
		});
	...
	// Remove Media
	$(document).on('click', '.vu_param_media .vu_param_m-btn[data-control="remove"]', function(e){
		e.preventDefault();
	...
	// Image Select Param
	$(document).on('click', '.vu_param_image-select .vu_param_is-images > span', function() {
		var $this = $(this),
			$input = $this.parents('.vu_param_image-select').find('input.vu_param_is-value');

		$this.parents('.vu_param_is-images').find('> span').removeClass('selected');
	...
	$(document).ajaxSuccess(function(e, xhr, settings) {
		if( settings.data !== undefined && settings.data.indexOf("action=vc_edit_form") != -1 ) {
			// Universal
			var $universal_params = $('.vc_shortcode-param[data-param_type="universal"]');
			...
			$media_params.each(function(){
				...
				try {
					media = JSON.parse(decodeURIComponent(escape(window.atob($this.find('.wpb_vc_param_value').val()))));
				} catch(err) {}

				if( ("id" in media) && ("url" in media) ) {
					$this.find('.vu_param_m-img').css({'background-image': 'url('+ media.url +')'});
			...
			$vu_param_select2.each(function() {
				...
				try {
					...
					$this.next('.select2').addClass('vu_param_select2-container' + (' ' + (options['extraCssClass'] || '')));

					$this.on('select2:open', function (evt) {
						$('body > .select2-container').addClass('vu_param_select2-outer-container');
					...
					if ( options.source ) {
						$.getJSON(options.source, function(data) {
							$.each(data, function(i, item) {
								$this.append('<option value="' + item.id + '"'+ ((item.selected == true) ? ' selected' : '') +'>' + item.text + '</option>');
							});
	...
	// Add More
	$(document).on('click', '.vu_param_universal .vu_param_u-add-item', function() {
		var $this = $(this),
	...
	// Delete
	$(document).on('click', '.vu_param_universal .vu_param_u-item-btn[data-control="delete"]', function() {
		var $this = $(this),
	...
	// Clone
	$(document).on('click', '.vu_param_universal .vu_param_u-item-btn[data-control="clone"]', function() {
		var $this = $(this),
			$container = $this.parents('.vu_param_universal'),
			$item = $this.parents('.vu_param_u-item-container');
	...
	// On change fields
	$(document).on('change', '.vu_param_universal input, .vu_param_universal select, .vu_param_universal textarea', function() {
		var $this = $(this),
			$container = $this.parents('.vu_param_universal');
	...
	var Universal = {
		toJson : function($el) {
			...
			$value.val(window.btoa(unescape(encodeURIComponent(JSON.stringify(value)))));
		},
		fromJson : function($el) {
			...
			try {
				var _value = vc.active_panel.model.attributes.params[$el.attr('data-vc-shortcode-param-name')];

				$el.find('.wpb_vc_param_value').val(_value);
				
				value = JSON.parse(decodeURIComponent(escape(window.atob(_value))));
			} catch(err) {}
			...
			for (var k in value) {
				...
				for (var name in value[k]) {
					var $input = $items.find('.vu_param_u-item-container').eq(k).find('[name="'+ name +'"]');

					if( $input.is('input:radio') || $input.is('input:checkbox') ){
						$input.filter('[value="'+ value[k][name] +'"]').attr({'checked':'checked'});
					} else {
						...
						if( $input.hasClass('vu_param_m-img-url') ) {
							if(value[k][name] != '') {
								...
								$items.find('.vu_param_u-item-container').eq(k).find('.vu_param_m-img').css({'background-image': 'url('+ value[k][name] +')'});
							}
						}
					}
				}
			...
			$items.attr({'data-count': $items.find('.vu_param_u-item-container').length});
		}
	}
})(jQuery);