<?php 
	...
	if ( preg_match('~\bbackground(-image)?\s*:(.*?)\(\s*(\'|")?(?<image>.*?)\3?\s*\)~i', $atts['css'], $match) ) {
		...
		if ( !empty($atts['bg_image']) ) {
			$atts['bg_image'] = housico_get_attachment_image_src($atts['bg_image'], 'full');
	...
	if ( $atts['enable_pattern'] == 'true' && !empty($atts['pattern_image']) ) {
		...
		$custom_css .= '.vu_row .'. esc_attr($custom_class) .'.vu_r-wrapper.vu_r-pattern-overlay:after{background-image:url('. esc_url(housico_get_attachment_image_src($atts['pattern_image'], 'full')) .');'. (!empty($atts['pattern_opacity']) ? 'opacity:'. esc_attr($atts['pattern_opacity']) : '') .'}';
	}
	...
	<div<?php echo !empty($atts['id']) ? ' id="'. esc_attr($atts['id']) .'"' : ''; ?> class="vu_row row vu_r-layout-<?php echo esc_attr($atts['content_layout']); ?>">
		<?php echo !empty($custom_css) ? '<style>'. $custom_css .'</style>' : ''; ?>
		<div class="vu_r-wrapper<?php housico_extra_class($atts['class']); ?>"<?php echo ($atts['enable_parallax'] == 'true' && !empty($atts['bg_image'])) ? ' data-parallax="scroll" data-image-src="'. esc_url($atts['bg_image']) .'"' : ''; ?><?php echo ($atts['bg_type'] == 'video' && !empty($atts['bg_video'])) ? ' data-property="'. esc_attr(json_encode($video_property)) .'"' : ''; ?><?php echo ($atts['css_animation'] != 'none' && $atts['css_animation'] != '') ? ' data-animation="'. esc_attr($atts['css_animation']) .'"' . (($atts['animation_delay'] > 0) ? ' data-delay="'. $atts['animation_delay'] .'"' : '') : ''; ?>>
	...
	echo housico_minify_output($output);
?>