<?php 
	...
	if( $this->getShortcode() === 'vc_tta_tabs' ) : ?>
		...
		<div class="vu_tabs wpb_tabs wpb_content_element vu_t-style-<?php echo esc_attr($atts['style']); ?> vu_t-nav-<?php echo esc_attr($atts['position']); ?> vu_t-nav-<?php echo esc_attr($atts['alignment']); ?><?php housico_extra_class($atts['class']); ?>" data-interval="<?php echo esc_attr($atts['autoplay']); ?>">
			<div class="vu_t-wrapper wpb_wrapper wpb_tour_tabs_wrapper ui-tabs vc_clearfix">
				...
				<?php 
					if( isset($sections_matches[5]) && !empty($sections_matches[5]) ) {
						...
						?>
							<div id="<?php echo esc_attr($section_atts['tab_id']); ?>" class="vu_t-panel wpb_tab ui-tabs-panel wpb_ui-tabs-hide vc_clearfix<?php housico_extra_class($section_atts['class']); ?>">
	...
	<?php elseif ( $this->getShortcode() === 'vc_tta_tour' ) : ?>
		<div class="vu_tour wpb_tour wpb_content_element default vu_t-nav-<?php echo esc_attr($atts['position']); ?><?php housico_extra_class($atts['class']); ?>" data-interval="<?php echo esc_attr($atts['autoplay']); ?>">
			<div class="vu_t-wrapper wpb_wrapper wpb_tour_tabs_wrapper ui-tabs vc_clearfix">
				<ul class="vu_t-nav wpb_tabs_nav ui-tabs-nav vc_clearfix">
					<?php 
						if( isset($sections_matches[3]) && !empty($sections_matches[3]) ) {
	...
	<?php else : ?>
		<div class="vu_accordion wpb_accordion wpb_content_element not-column-inherit<?php housico_extra_class($atts['class']); ?>" data-collapsible="<?php echo (isset($atts['collapsible_all']) && !empty($atts['collapsible_all'])) ? 'yes' : 'no'; ?>" data-vc-disable-keydown="false" data-active-tab="<?php echo absint($atts['active_section']); ?>">
