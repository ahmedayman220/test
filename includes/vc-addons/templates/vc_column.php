<?php 
	...
	<div<?php echo !empty($atts['el_id']) ? ' id="'. esc_attr($atts['el_id']) .'"' : ''; ?> class="vu_column<?php housico_extra_class($atts['el_class']); ?>"<?php echo ($atts['css_animation'] != 'none' && $atts['css_animation'] != '') ? ' data-animation="'. esc_attr($atts['css_animation']) .'"' . (($atts['animation_delay'] > 0) ? ' data-delay="'. $atts['animation_delay'] .'"' : '') : ''; ?>>
